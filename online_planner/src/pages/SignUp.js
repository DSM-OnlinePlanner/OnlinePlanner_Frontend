...
function SignUp(){
    ...
    const onSignUp = async () => {
        if(emailCheck === ''){
            setErrorMessage(setAllErrorMessage, "이메일 인증을 해야합니다.", ErrorType.ALL);
            return;
        }
        ...
        await axios
            ...
            .catch((error) => {
                setErrorMessage(setEmailCheckErrorMessage, "인증코드가 맞지 않습니다.", ErrorType.EMAIL_CHECK);
            })
        ...
        const onSignUpSuccess = (response) => {
            axios
                ...
                .catch((error) => {
                    setErrorMessage(setEmailCheckErrorMessage, "사용자 생성에 실패했습니다.", ErrorType.EMAIL_CHECK);
                })
    }
    ...
    const onClickEmailCheck = async () => {
        ...
        if(!mailformat.test(email)){
            setErrorMessage(setEmailErrorMessage, '이메일 형식이 맞지 않습니다.', ErrorType.EMAIL);
            isFailed = true;
        }
        ...
        if((pw.length < 8 || pw.length > 20) || (pw.search(/\s/) !== -1) || (num < 0 || eng < 0 || spe < 0)){
            setErrorMessage(setPasswordErrorMessage, '유효하지 않은 비밀번호 형식입니다.', ErrorType.PASSWORD);
            isFailed = true;
        }

        if(password !== passwordCheck){
            setErrorMessage(setPasswordCheckErrorMessage, '비밀번호를 확인해주세요.', ErrorType.PASSWORD_CHECK);
            isFailed = true;
        }
        ...
        const param = { //이메일 보낼 파라미터
            email: email,
            name: nickname
        }

        const emailResend = () => {axios.put('/api/mail', null, {params: param})
        .then(onEmailSentSuccess)
        .catch(error => { //메일 보내기 실패
            setErrorMessage(setEmailCheckErrorMessage, '다시 시도해주세요.', ErrorType.EMAIL_CHECK);
            console.log(error);
        })};

        if(!isEmailCheckSent){ //첫번째 보내는 이메일 인증일 경우
            await axios.post('/api/mail', null, {params: param})
            .then(onEmailSentSuccess)
            .catch(error => { //메일 보내기 실패
                emailResend();
                console.log(error);
    ...
    const onEmailSentSuccess = (response) => {
        if(!isEmailCheckSent){ //첫번째 보내는 이메일일때
    ...
    return (
        <div className='signin-container use-animation'>
            ...
            <div className='display-flex justify-content-space-between'>
                ...
                <span id ={ErrorType.PASSWORD} className='font-16px color-red font-medium opacity-0'>{passwordErrorMessage}</span>
            </div>
            ...
            <div className='signin-content use-animation'>
            <div className='display-flex justify-content-space-between'><span className='textfield-title font-16px color-gray3 font-medium'>비밀번호 확인</span> <span id ={ErrorType.PASSWORD_CHECK} className='font-16px color-red font-medium opacity-0'>{passwordCheckErrorMessage}</span></div>
            ...
            <div className='signin-content use-animation'>
            <div className='display-flex justify-content-space-between'><span className='textfield-title font-16px color-gray3 font-medium'>닉네임</span> <span id ={ErrorType.NICKNAME} className='font-16px color-red font-medium opacity-0'>{nicknameErrorMessage}</span></div>
            ...
            <div className='use-animation signin-content'>
                <div className='display-flex justify-content-space-between'><span className='textfield-title font-16px color-gray3 font-medium'>이메일 인증</span> <span id={ErrorType.EMAIL_CHECK} className='font-16px color-red font-medium opacity-0'>{emailCheckErrorMessage}</span></div>
            ...
            <div id={ErrorType.ALL} className='font-18px color-red font-medium opacity-0'>{allErrorMessage}</div>
            <button className="border-radius-10px background-blue font-24px font-medium color-white login-button use-animation" onClick={onSignUp}>
                회원가입
            </button>
