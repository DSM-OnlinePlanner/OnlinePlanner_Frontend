...
function TodoComponent(props){
    ...
    const getRelativeTime = (time) => {
        ...
        if(nowTime.getFullYear() === time.getFullYear() && nowTime.getMonth() === time.getMonth()){
            //오늘이면
            if( nowTime.getDate() === time.getDate()){ 
                str = time.getHours() + ":00";
            }
            //어제면
    ...
    return(
        <>
        <S.TodoContainer color={ChangeResponsibilityToColor(planner.want, planner.success)} onClick={() => {setIsModalActive(true)}}>
            <S.TodoInner status={planner.success}>
                <S.TodoTitle>
                    <span>{planner.title}</span><span>중요도 : {planner.priority}</span>
                </S.TodoTitle>
                <S.TodoDiv>
                    <span>{planner.content}</span> <span>{getTimeFromStartEndTime(planner.startDate, planner.endDate)}</span>
                </S.TodoDiv>
            </S.TodoInner>
        </S.TodoContainer>
        {isModalActive && <TodoModal key={props.key} planner={planner} activefunction={setIsModalActive}/>}
        </>
